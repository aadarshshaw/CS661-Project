...
df_filtered= pd.read_csv(Path(__file__).parents[2] 
                         / "Datasets" / "CO2_Emissions" / "CO2_emission_by_countries.csv")
df = pd.read_csv(Path(__file__).parents[2] 
                         / "Datasets" / "CO2_Emissions" / "co-emissions-per-capita.csv")
...
def Countries_emitting_most_CO2():
    ...
    for country in total_emissions['Entity']:
        ...
        fig1.add_trace(go.Scatter(x=country_data['Year'], y=country_data['Annual CO2'], mode='lines', name=country))

    # Update layout
    fig1.update_layout(title='Annual CO2 Emission by top 8 countries',
                    xaxis_title='Year',
...
def Percentage_Share_of_CO2_per_country():
    ...
    for country in top_emitters:
        ...
        share = (df_filtered[df_filtered['Entity'] == country].groupby('Year')['Annual CO2'].sum() / total_emissions_yearly) * 100
        fig.add_trace(go.Scatter(x=share.index, y=share, mode='lines', name=country))

    # Update layout
    fig.update_layout(title='Share of CO2 Emissions by Top 8 Countries',
                    xaxis_title='Year',
...
def World_CO2_emission():
    ...
    # Group the data by year and calculate the total CO2 emissions for each year
    total_world_emissions = df_filtered.groupby('Year')['Annual CO2'].sum().reset_index()

    # Create a Plotly graph
    fig1 = go.Figure()

    # Plot total world emissions
    fig1.add_trace(go.Scatter(x=total_world_emissions['Year'], y=total_world_emissions['Annual CO2'], mode='lines', name='World', line=dict(color='black')))

    # Update layout
    fig1.update_layout(title='Annual CO2 Emission by World',
                    xaxis_title='Year',
                    yaxis_title='Annual CO2 emission',
                    hovermode='x unified',
                    plot_bgcolor="rgba(0, 0, 0, 0)",
    ...
    [
        ...
        dmc.Grid(
            children=[
                ...
                dmc.Col([
                    dcc.Graph(id="mini-graph-container", figure=world_CO2_map(None)[1])
                ], span=12),
                dmc.Col(
                    dcc.Graph(
